...
Portrait and landscape browser arcade game, in English. Solo Olympic tour (30 Games, 1896 Athens → 2024 Paris, 3 arcade lives) plus realtime multiplayer for up to 5 phones. No build step; the browser client has no runtime dependencies.
...
- `dist/engine.js`: deterministic 120 Hz simulation; cadence, speed, jump arc, interpolated hurdle/finish crossings, seeded AI, pause and snapshots. `reset(seed, humanIds)` and `setHuman(id, human)` support net hosts.
...
`npm test` runs gameplay tests for cadence, duplicate input, jump timing, falling, full races, pause, snapshot/restart and frame-rate independence, plus arcade lives and multiplayer room/input/snapshot/NetView rules.
